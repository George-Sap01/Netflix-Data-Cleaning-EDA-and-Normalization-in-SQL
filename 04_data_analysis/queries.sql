...
from ( 
	select name, num, dense_rank() over(order by num desc) as dr from movies_table
) as a
where dr < 11
UNION 
select name, num as 'number of appearances in series/movies in netflix', 'Series'
from (
	select name, num, dense_rank() over(order by num desc) as dr from series_table
) as a
where dr < 11;
...
(
	select 	id
	from ( 
		select id, dense_rank() over(order by num desc) as dr from movies_table
	) as a
	where dr < 11
	UNION 
	select 	id
	from (
		select id, dense_rank() over(order by num desc) as dr from series_table
	) as a
	where dr < 11
)
